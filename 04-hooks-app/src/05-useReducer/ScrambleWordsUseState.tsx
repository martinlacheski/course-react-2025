...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    // Si se supera el número máximo de errores
    if (errorCounter + 1 >= maxAllowErrors) {
      setIsGameOver(true);
    }

    // Si se supera el número máximo de saltos
    if (skipCounter >= maxSkips) return;

    // Desordenar próxima palabra usando shuffleArray
    setCurrentWord(shuffleArray(words)[0]);
    setScrambledWord(scrambleWord(shuffleArray(words)[0]));
    setGuess("");
  };
