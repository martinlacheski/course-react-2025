...
export const TasksApp = () => {
  ...
  // Se desestructuran los valores del estado
  const { todos, length: totalCount, completed: completedCount } = state;

  const addTodo = () => {
    if (inputValue.trim().length === 0) return;
    // Se dispara la acción de agregar una tarea
    dispatch({ type: "add", payload: inputValue.trim() });
    setInputValue("");
  };

  const toggleTodo = (id: number) => {
    // Se dispara la acción de cambiar el estado de una tarea
    dispatch({ type: "toggle", payload: id });
  };

  const deleteTodo = (id: number) => {
    // Se dispara la acción de eliminar una tarea
